  ...
  const columns: Column<Lead>[] = [
    ...
    { 
      ...
      render: (row) => {
        ...
        if (row.sync_status === 'failed') return <Badge variant="danger">❌ فشل</Badge>;
        return <Badge variant="warning">⏳ في الانتظار</Badge>;
      }
    },
    { key: 'deal_id', header: 'معرف الصفقة', render: (row) => row.deal_id || '-' },
    { key: 'created_at', header: 'التاريخ', render: (row) => new Date(row.created_at).toLocaleDateString('ar-SA') },
    {
      key: 'actions',
      header: 'الإجراء',
      render: (row) => (
